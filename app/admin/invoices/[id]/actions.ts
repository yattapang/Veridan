...
export async function regenerateInvoice(invoiceId: string): Promise<RegenerateInvoiceResult> {
  ...
  // Standalone invoices have no source quote (quote_id is null — see
  // 20260807000003_standalone_invoices.sql) and are never generated by
  // generateDepositInvoiceForQuote/generateBalanceInvoiceForQuote below, so
  // "regenerate" has nothing to regenerate FROM. A void standalone invoice
  // is a dead end by design (same as any other void invoice that isn't
  // reissued) — create a fresh standalone invoice instead.
  if (invoice.invoice_type === "standalone" || !invoice.quote_id) {
    return {
      ok: false,
      error: "This is a standalone invoice with no source quote — create a new standalone invoice instead of regenerating this one.",
    };
  }
